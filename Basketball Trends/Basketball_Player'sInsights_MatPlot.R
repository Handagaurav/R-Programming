...
#Visualization using user defined function
myplot <- function(data,rows=1:10){
  Data <- data[rows,,drop=F]
  matplot(t(Data),type = "b",pch = 15:18,col = c(1:4,6))
  legend("topright",inset=c(-0.45,0),legend = Players[rows], col = c(1:4,6),pch = 15:18,horiz = F)
}
...
#Salary
myplot(Salary)
myplot(Salary/Games) #They were injured for few games, so the have a overall high value
myplot(Salary/FieldGoals)

#As salary is not affected bcoz of injuries, but in-game statistics does. Therefore, we plot is separately
#In-game metrics
...
myplot(FieldGoals/FieldGoalAttempts) #Player Accuracy
myplot(FieldGoalAttempts/Games) #Successful Attempts
myplot(Points/Games) 

#Interesting Observation
myplot(MinutesPlayed/Games) #It's dropping over the years.
myplot(Games)
...
#In basketball, you can either score 3 or 2 points. But the data that we have also includes
#points for free throws which has to be subtracted from Points dataset. That is why the avg 
# of points per player is seen to be more than 3.